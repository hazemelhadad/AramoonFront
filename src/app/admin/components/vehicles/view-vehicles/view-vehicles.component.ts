...
export class ViewVehiclesComponent implements OnInit, AfterViewInit {
  ...
  editDialog(data: any) {
    const dialogRef = this.dialog.open(EditVehicleDialogComponent, { data });
    dialogRef.afterClosed().subscribe((result) => {
      console.log(`Dialog result: ${result}`);
      if (result === 'success') {
        this.getMethod();
  ...
  deleteVehicle(pltNum: string): void {
    this._service.deleteVehicle(pltNum).subscribe(
      () => {
        alert(`Vehicle with Plate Number ${pltNum} deleted successfully`);
        this.getMethod();
      },
      (error) => {
